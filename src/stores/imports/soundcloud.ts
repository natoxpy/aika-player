...
import { type Artist, getArtists, getAvatar, getDefaultAvatar } from '../api/crud/artists'
import { uploadFromUrl } from '../api/fs/upload'
import { type SoundcloudImportMusic, previewFromUrl } from '../api/import/soundcloud'
import { getAlbums, type Album, getAlbumDefaultCover } from '../api/crud/albums'
...
export const useSoundcloudImport = defineStore('soundcloud_imports', () => {
     ...
     * */
    const artists = ref<Map<string, Array<[string, boolean, boolean]>>>(new Map())
    ...
    const addImportFromUrl = async (url: string) => {
        ...
        addArtistFromPreview(musicId, track.author.username)
    ...
    const addArtistFromPreview = async (musicId: string, name: string) => {
        ...
        artistsNotIndb.value.set(artistId, name)
        artists.value.set(musicId, [[artistId, false, false]])
    }
    ...
    return {
        ...
        addArtist: (musicId: string, artistId: string, featured = false, existsIndb = true) => {
            checkSession()
        ...
        getArtistsForSelector: async (music_id: string, filter?: string) => {
            ...
            const apiArtists = (await allArtists()).filter((item) => !used.has(item.id))
        ...
        getAlbumsForSelector: async (music_id: string, filter?: string) => {
            ...
            const apiAlbums = (await allAlbums()).filter((item) => !used.has(item.id))
